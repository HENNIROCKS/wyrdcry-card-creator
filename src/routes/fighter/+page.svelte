...
<script lang="ts">
	...
	const defaultData: FighterCardData = {
		...
		health: '10',
		bravery: '5',
		weapons: [{ name: 'Unarmed', range: '1', attacks: '2', damage: '1/2' }],
		talentsText: '**[Double] Driven by Greed**: A fighter can only use this ability if it is carrying, or is within 1" of a treasure token. Add 1 to the Fight and Attacks characteristic of this fighter’s next melee attack action.',
		keywords: ['Human', 'Mercenaries', 'Henchman'],
	...
	function makeSlug() {
		...
		const parts = [toSlug(data.name || 'fighter')];
		const now = new Date();
	...
	>
		...
		<div class="flex items-center justify-between border-b border-zinc-800 px-5 py-4 shrink-0">
			<div class="flex items-center gap-3">
				...
				<h1 class="text-sm font-semibold tracking-widest uppercase text-zinc-200">{t('ui.fighter-card')}</h1>
			</div>
